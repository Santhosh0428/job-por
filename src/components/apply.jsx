...
const schema=z.object({
    experience:z.number().min(0,{message:"Experience must be atleast 0"})
    .int(),
    skills:z.string().min(3,{message:"Enter atleast 3 skills"}),
...
const Apply = ({ user, job, applied = false, fetchJob }) => {
  ...
  return (
    <Drawer open={applied ? false : undefined}>
      <DrawerTrigger asChild>
        <Button
          size="lg"
          variant={job?.isOpen && !applied ? "aqua" : "red"}
      ...
      <DrawerContent>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 p-4 pb-0">
          <Input
            ...
            className="flex-1" {...register("experience",{
                valueAsNumber:true,
            })}
          />
          {errors.experience && (
          ...
          <Input
            type="file"
            accept=".pdf,.doc,.docx"
            className="flex-1 file:text-gray-600" {...register("resume")}
          />
          {errors.resume && (
          ...
          {loadingApply && <BarLoader width={"100%"} color="aquamarine" />}
